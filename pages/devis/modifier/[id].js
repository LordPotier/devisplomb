      ...
      supabase.from('devis').select('*').eq('id', id).single().then(({ data }) => {
        if (data) {
          ...
          setForm({
            ...
            majoration: data.majoration || 0
          })
          setIaText(data.intro_ia || '')
          // Charger les lignes existantes
          const lignes = data.lignes || []
          const predefined = lignes.filter(l => PRESTATIONS.find(p => p.nom === l.nom))
          const custom = lignes.filter(l => !PRESTATIONS.find(p => p.nom === l.nom))
    ...
    <Layout active="/devis">
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{devis.numero} — {devis.client_nom}</p>
        </div>
        <div className="flex gap-3">
          <button onClick={() => router.push(`/devis/${id}`)}
            className="border border-gray-200 px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-50">
            Annuler
          </button>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            {customLines.length > 0 && (
              <div className="mb-4 space-y-2">
                <p className="text-xs text-gray-500">Prestations personnalisées</p>
                {customLines.map((l, i) => (
                  <div key={i} className="flex justify-between items-center bg-gray-50 rounded-lg px-3 py-2 text-sm">
                    <span>{l.nom}</span>
                    <div className="flex items-center gap-2">
                      <span className="font-medium">{l.prix} €</span>
                      <button onClick={() => removeCustom(i)} className="text-red-500 hover:text-red-700">✕</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <div className="bg-gray-50 rounded-lg p-4 text-sm">
              <div className="flex justify-between mb-3">
                <div><div className="font-semibold text-base">DEVIS</div><div className="text-gray-400 text-xs">{devis.numero} · {new Date(devis.created_at).toLocaleDateString('fr-FR')}</div></div>
